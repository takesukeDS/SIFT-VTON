...
def get_tvloss(coords, mask, ch, cw, dim=None):
    b, n, _ = coords.shape
    ...
    y_mask_sum = y_mask.sum(dim=dim)
    x_mask_sum = x_mask.sum(dim=dim)
    # some data does not have valid regions, avoid nan
    # in such cases, atv loss is skipped because y_tvloss and/or x_tvloss are all near zeros
    y_mask_sum = torch.where(y_mask_sum < 1e-6, torch.ones_like(y_mask_sum), y_mask_sum)
    x_mask_sum = torch.where(x_mask_sum < 1e-6, torch.ones_like(x_mask_sum), x_mask_sum)
    tv_loss = y_tvloss.sum(dim=dim) / y_mask_sum + x_tvloss.sum(dim=dim) / x_mask_sum
    return tv_loss
...
class ExplicitWarpAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0., **kwargs):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
        ...
        if exists(mask1) or exists(mask2):  # [BS x 1 x H x W] float
            if mask1.ndim == 4 and mask2.ndim == 4:
                ...
                if mH != 8:
                    ...
                    attn_mask = mask1.reshape(bs, -1).unsqueeze(-1) * mask2.reshape(bs, -1).unsqueeze(
                        1)  # [BS x HW x hw]
                    attn_mask = repeat(attn_mask, "b HW hw -> (b h) HW hw", h=h)
...
class HybridWarpAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, zero_init=False, **kwargs):
        super().__init__()
        print(f"Setting up {self.__class__.__name__}. Query dim is {query_dim}, context_dim is {context_dim} and using "
              f"{heads} heads.")
    ...
    def forward(
            ...
            use_attention_tv_loss=False,
            use_loss=True,
            mask3=None, explicit_sim=None, explicit_ratio=1.0,
            **kwargs
    ):
        q = self.to_q(x)
        is_self_attn = context is None
        ...
        sim = sim.softmax(dim=-1)
        h = self.heads
        if use_attention_tv_loss and key_token_length > 700 and (
        not is_self_attn) and key_token_length < 3000 and use_loss:
            _, HW, hw = sim.shape
            dx = int((HW // 12) ** 0.5)
            mH = int(4 * dx)
            mW = int(3 * dx)
        ...
        if not is_self_attn:
            if mask3 is None or explicit_sim is None:
                raise NotImplementedError("mask3 and explicit_sim must be provided for HybridWarpAttention")
            mask3 = mask3.bool()
            mask3 = mask3.expand(-1, h, -1, -1)  # [BS x h x mH x mW]
            mask3 = mask3.reshape(sim.shape[:2])
            explicit_sim = torch.repeat_interleave(explicit_sim, h, dim=0)
            sim_tmp = torch.zeros_like(explicit_sim, dtype=explicit_sim.dtype, device=explicit_sim.device)
            sim_tmp[mask3] = explicit_sim[mask3]
            sim = sim_tmp * explicit_ratio + sim * (1 - explicit_ratio)
...
class CrossAttentionSIFTLoss(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, zero_init=False, **kwargs):
        super().__init__()
        print(f"Setting up {self.__class__.__name__}. Query dim is {query_dim}, context_dim is {context_dim} and using "
              f"{heads} heads.")
    ...
    def forward(
        ...
        if use_sift_loss and (hist_mask is None or explicit_sim is None) and use_loss:
            raise NotImplementedError("hist_mask must be provided when use_sift_loss is True")
        q = self.to_q(x)
        is_self_attn = context is None
        context = default(context, x)
        k = self.to_k(context)
        ...
        not is_self_attn) and key_token_length < 3000 and use_loss:
            ...
            # mask1 is negated in apply_model() by cldm.mask_resize
            # and here negated again... terrible code from StableVITON
            tv_loss = get_tvloss(weighted_centered_grid_hw, ~mask1_repeat, ch=mh, cw=mw)
            attn_loss = tv_loss * tv_loss_scale

        if use_sift_loss and (not is_self_attn) and use_loss and explicit_sim is not None:
            # hist_mask : [BS x mH x mW]
            loss = -explicit_sim.unsqueeze(1) * torch.log(
                sim.view(b, h, hist_mask.shape[1] * hist_mask.shape[2], -1) + 1e-8)
            loss = loss.sum()
            hist_mask_sum = hist_mask.sum()
            if hist_mask_sum > 0:
                loss = loss / (b * h * hist_mask_sum)
            attn_loss = attn_loss + loss * sift_loss_scale
...
class CrossAttentionSIFTLossAVE(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0, zero_init=False, **kwargs):
        super().__init__()
        print(f"Setting up {self.__class__.__name__}. Query dim is {query_dim}, context_dim is {context_dim} and using "
              f"{heads} heads.")
    ...
    def forward(
        ...
        tv_loss = torch.zeros(b, dtype=x.dtype, device=x.device)
        sift_loss = torch.zeros(b, dtype=x.dtype, device=x.device)
        sim = einsum('b i d, b j d -> b i j', q, k) * (self.dim_head ** -0.5)
        sim = sim.softmax(dim=-1)
        h = self.heads
        if use_attention_tv_loss and key_token_length > 700 and (
        not is_self_attn) and key_token_length < 3000 and use_loss:
            ...
            # mask1 is negated in apply_model() by cldm.mask_resize
            # and here negated again... terrible code from StableVITON
            tv_loss = get_tvloss(weighted_centered_grid_hw, ~mask1_repeat, ch=mh, cw=mw, dim=(1, 2, 3))
            tv_loss = tv_loss * tv_loss_scale

        if use_sift_loss and (not is_self_attn) and use_loss and explicit_sim is not None:
            # hist_mask : [BS x mH x mW]
            # if there is no matching, explicit_sim will be zero, so loss is.
            loss = -explicit_sim.unsqueeze(1) * torch.log(
                sim.view(b, h, hist_mask.shape[1] * hist_mask.shape[2], -1) + 1e-8)
            loss = loss.sum(dim=(1, 2, 3))
            hist_mask_sum = hist_mask.sum(dim=(1, 2))
            hist_mask_sum = torch.where(hist_mask_sum == 0, 1, hist_mask_sum)
            loss_weights = 1/(h * hist_mask_sum)
            loss = loss * loss_weights
            sift_loss = loss * sift_loss_scale

        if exists(mask):
            raise NotImplementedError
        out = einsum('b i j, b j d -> b i d', sim, v)
        out = rearrange(out, '(b h) n d -> b n (h d)', h=h)
        return self.to_out(out), tv_loss, sift_loss

class CrossAttentionVis(CrossAttention):
    def forward(
            self,
            x,
            context=None,
        ...
        if use_attention_tv_loss or use_sift_loss:
            output = output + (attn_loss,)
        output = output + (sim,)
        return output


class CrossAttentionSIFTLossVis(CrossAttentionSIFTLoss):
    def forward(
            self,
            x,
            context=None,
